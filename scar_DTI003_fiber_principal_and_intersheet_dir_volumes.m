...
addpath('C:\Users\petnov\Dropbox\geometry\');
...
H_mesh.tricentres=csvread('D:\ARVC meshing automatic\patients\DTI003\DTI003_mesh\DTI003_coarse_tetrahedronCenters.csv');
...
RV_anterior=0;
  if RV_posterior==1
            ...
            elseif RV_anterior==1
                ...
                y=-11.14;
                z=-6.5;
           elseif exist("cursor_info")==1
               disp("starting point of scar has already been selected");
             
            else
                disp("please select a point on the mesh");
                while(1)
         ...
         mid1 =[x y z];
             for i=1:Ni
                 [rw,col]=find(H_mesh.tri==mid1_id);
                  fiber_dir = -H_mesh.triORTHOfull(col(1),7:9);%use the direction in the fiber dir
                  if RV_anterior ==1
                      fiber_dir=-fiber_dir;
                  end
                % fiber_dir = -fibers_normal(col(1),1:3);
                 mid1 = mid1+(fiber_dir*i)/Ni;
                  id_next=knnsearch(H_mesh.xyz,mid1);
                  
                  points = [points,id_next];
                 dists= distance(H_mesh.xyz,mid1);
                 keep=find(dists<ri);%keep the points which are a radial dist of rp away
...
otherwise
  ts_scaling_no_fib=ones(23,1);
        ...
        mid1 = endo1+AB/2; %finding a mid point between two points, A(endo point) and B (epi)
         ...
         H_mesh.lge=no_lge;
          scar_volumes_H_mesh_fcn(H_mesh);
