 ...
 *
 * Modified by Mikael Wallman <miwa@fcc.chalmers.se>, 
 * in 2011 to accept several starting points with different times.
 *
             ...
             FibHeap  *theHeap  )
{
   ...
   for (i=0; i<M; i++) 
   {
      ...
      { 
          ...
          //mexPrintf( "Put start node S=%d on the que\n" ,S[j] );//TT
          if ( j<(MS-1) ) ++j;  
...
//                  whichneighbor, olddist, closestD+arclength, closest );//TT
   ...
   // source nodes have no parent
   for (i=0; i<MS; ++i) P[S[i]] = -1;
...
{
  double    *sr,*D,*P,*SS,*Dsmall,*T;
  double *Psmall;
  mwIndex       *irs,*jcs;
  mwSize  M,N,MS,NS,i,j,in,MT,NT;
  long int *S, lastS;
  ...
  if (nrhs < 2 || nrhs > 3 )
  {
   ...
   if (M != N) mexErrMsgTxt( "Input matrix (1st argument) needs to be square." );
    
   SS = mxGetPr(prhs[1]);  // pointer to start nodes 
   MS = mxGetM( prhs[1] ); 
   NS = mxGetN( prhs[1] );
     
   if ((MS==0) || (NS==0) || ((MS>1) && (NS>1))) mexErrMsgTxt( "Source nodes (2nd argument) are specified in one dimensional matrix only" );
   if (NS>MS) MS=NS;
   
   if ( nrhs == 3 )
   {
        T = mxGetPr(prhs[2]);  // pointer to start nodes 
        MT = mxGetM( prhs[2] ); 
        NT = mxGetN( prhs[2] );
   }
   else
   {
        T = (double *) mxCalloc( MS , sizeof(double));
        MT = MS;
        NT = 1;
   }
   
   if ((MT==0) || (NT==0) || ((MT>1) && (NT>1))) mexErrMsgTxt( "Source times (3rd argument) are specified in one dimensional matrix only" );
   if (NT>MT) MT=NT;
   if (MT!=MS) mexErrMsgTxt( "Source times (3rd argument) must have the same size as source nodes (2nd argument)" );
   
   
   
   S = (long int *) mxCalloc( MS , sizeof(long int));
   ...
   if (mxIsSparse( prhs[ 0 ] ) == 1)
   {
     ...
     for (i=0; i<1; i++)
     {
        ...
        {
            S[j] = (long int) SS[j]-1;
            if (S[j]<lastS) mexErrMsgTxt( "Multiple source nodes must be given in ascending order" );
            lastS = S[j];
            if ((S[j] < 0) || (S[j] > M-1)) mexErrMsgTxt( "Source node(s) out of bound" );
        }
        /* -------------------------------------------------------------------------------------------------
        ...
        dodijk_sparse( M,N,S,MS,T,MT,P,D,sr,irs,jcs,A,theHeap );
